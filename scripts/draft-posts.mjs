...
function buildAliasLookup(knownIds) {
  ...
  for (const id of knownIds) {
    ...
    for (const a of aliases) lookup[a.toLowerCase()] = id
  }
  return lookup
}
...
function parseFilename(filename) {
  // Prophet_Name_YYYYMMDD-YYYYMMDD_YYYYMMDD_HHMMSS.md
  ...
  const prophetHint  = prophetParts.join(' ')   // e.g. "Brandon Biggs"

  let pubDate = new Date().toISOString().slice(0, 10)
  if (dateRangeIdx >= 0) {
    const end = parts[dateRangeIdx].split('-')[1]
    pubDate = `${end.slice(0,4)}-${end.slice(4,6)}-${end.slice(6,8)}`
  }

  return { prophetHint, pubDate }
}
...
async function processDraft(sourceFile, keys, knownIds, aliasLookup) {
  const raw = readFileSync(join(SOURCE_DIR, sourceFile), 'utf-8')
  const { prophetHint, pubDate } = parseFilename(sourceFile)
...
以下是一份來自「${prophetHint}」的 Podcast 腳本，收錄日期：${pubDate}：
...
- matchedProphetIds：對應到以下名冊的 ID（嚴格只用名冊裡的字串，對應不到就空陣列）
  ...
  賽巴斯帝安/Sebastian→3036、Dienach→3906
...
回傳的 JSON 結構：
{
  ...
  const allUnidentified = [...new Set([...unidentifiedPeople, ...clientUnmatched])]
    .filter(name => !allMatchedIds.length || !prophetNamesInText.includes(name) || !verifiedFromAI.some(id => ALIASES[id]?.includes(name.toLowerCase())))
...
async function main() {
  ...
  for (const file of toProcess) {
    console.log(`\n🔄 ${file}`)
    try {
      const result = await processDraft(file, keys, knownIds, aliasLookup)
      tracking[file] = {
        status:    'drafted',
      ...
      console.log(`   ✅ drafts/${result.draftFile}`)
    ...
    await new Promise(r => setTimeout(r, 2000))  // 避免 rate limit
  }
  ...
  console.log('\n' + '═'.repeat(55))
  console.log(`📋 本次完成報告`)
  console.log(`   處理：${report.length} 篇 | 成功：${ok.length} 篇 | 失敗：${failed.length} 篇`)

  if (ok.length) {
    console.log('\n✅ 草稿清單：')
    for (const r of ok) {
      const ids = r.matchedIds?.length ? r.matchedIds.join('、') : '（未識別）'
      console.log(`   • drafts/${r.draftFile}`)
      console.log(`     預言家：${ids}  |  預言 ${r.predictionCount} 條`)
    }
  }

  if (needsHuman.length) {
    console.log('\n⚠️  以下草稿需人工確認 prophet 欄位：')
    for (const r of needsHuman) {
      const issues = []
      if (r.matchedIds?.length === 0) issues.push('未能自動識別預言家')
      if (r.unidentified?.length)     issues.push(`未識別人物：${r.unidentified.join('、')}`)
