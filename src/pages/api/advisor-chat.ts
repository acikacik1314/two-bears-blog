...
當你判斷資訊足夠時，嚴格只輸出以下 JSON，無其他文字：
{
  ...
  "summary": {
    "destination": "城市名稱",
    "companions": "旅伴關係（情侶/家庭/朋友/獨自/長輩）",
    "comfort": "舒適型或經濟型",
...
const DIRECT_SYSTEM = `使用者輸入一個飯店名稱（可能是中文或英文）。
識別是哪一間飯店，回傳其正式英文名稱和所在城市（英文城市名，例如 Taipei、Tokyo、Singapore）。

嚴格只輸出 JSON，不要任何其他文字：
找到時：{"found":true,"name":"飯店中文慣用名","en":"正式英文全名","city":"城市英文名"}
找不到或不確定時：{"found":false}`;
...
請推薦 4 間最適合的飯店。`;
  ...
  if (body.type === 'direct') {
    ...
    if (!hotelName) return json({ ok: false, error: '請輸入飯店名稱' });

    for (const key of shuffled) {
      const r = await callGemini(
        key, DIRECT_SYSTEM,
        [{ role: 'user', parts: [{ text: hotelName }] }],
        true,
      );
      if (!r.ok || !r.text) continue;
      const parsed = parseJSON(r.text) as any;
      if (parsed?.found === false) {
        return json({ ok: false, error: '找不到這間飯店，請試試完整名稱或英文名' });
      }
      if (parsed?.found && parsed?.en && parsed?.city) {
        return json({ ok: true, hotel: { name: parsed.name || hotelName, en: parsed.en, city: parsed.city } });
      }
    }
    return json({ ok: false, error: '暫時無法查詢，請稍後再試 🐻' });
  }
