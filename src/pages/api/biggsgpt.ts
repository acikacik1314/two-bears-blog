...
export async function POST({ request }: APIContext) {
  ...
  if (getGeminiKeys().length) {
    ...
    // Fall through to Groq when Gemini is exhausted
  }

  // Fallback: Groq (no live search, but always available)
  if (getGroqKeys().length) {
    ...
    const result = await callGroq(messages, { maxTokens: 1024, temperature: 0.9 })
    if (result.text && result.text !== '__RATE_LIMITED__') {
