...
【address / phone / price_per_person 規則】
- address：填完整地址（含城市），查不到則填空字串 ""
- phone：填當地格式電話號碼，查不到則填空字串 ""
- price_per_person：填每人均消範圍，例如 "NT$150–300" 或 "¥800–1200"，查不到則填 ""
  ...
  "must_eat": [
    {
      ...
      "is_vegetarian": false,
      "confirmed_open": true,
      "address": "台南市中西區民族路2段16號",
      "phone": "06-222-3456",
      "price_per_person": "NT$100–200",
      "tags": ["在地人", "平價"]
    }
  ],
  "hidden_gems": [
...
function sanitizeRestaurant(r: any): Restaurant {
  return {
    ...
    address: String(r?.address ?? '').slice(0, 80),
    phone: String(r?.phone ?? '').slice(0, 20),
    price_per_person: String(r?.price_per_person ?? '').slice(0, 20),
